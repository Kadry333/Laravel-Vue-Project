...
const form = useForm({
    name: "",
    manager_id: user?.roles?.some((role) => role.name === "admin")
        ? null
        : user?.value?.id,
});

function submit() {
    form.post("/admins/floors", {
        onSuccess: () => {
            form.reset();
        },
    });
...
<template>
    <div class="w-full">
        <div class="mb-6 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-slate-900">Add New Floor</h1>
                <p class="text-sm text-slate-500 mt-1">
                    Fill in the details to create a new floor.
                </p>
            </div>
        </div>

        <div
            class="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-5 w-full"
        >
            <div class="space-y-2">
                <Label>Name</Label>
                <Input
                    v-model="form.name"
                    placeholder="e.g. El-Mahalla Floor"
                    required
                    type="text"
            ...
            <div class="space-y-2" v-if="isAdmin">
                ...
                <p v-if="form.errors.manager_id" class="text-xs text-red-500">
            ...
            <div class="flex gap-2 justify-end pt-2">
                ...
                <Button @click="submit" :disabled="form.processing">
                    {{ form.processing ? "Saving..." : "Create Floor" }}
                </Button>
            </div>
        </div>
    </div>
