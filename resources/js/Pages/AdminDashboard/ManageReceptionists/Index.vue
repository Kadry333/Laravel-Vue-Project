...
const actionCopy = computed(() => {
    if (actionType.value === "ban") {
        return {
            ...
            description:
                "This receptionist will be signed out and blocked from protected routes until you unban the account.",
    ...
    if (actionType.value === "unban") {
        return {
            ...
            description:
                "This receptionist will regain access to the system immediately after signing in again.",
    ...
    return {
        ...
        description:
            "This will permanently remove the receptionist account and its uploaded avatar.",
        ...
        .post(
            route(
                actionType.value === "ban"
                    ? "admins.receptionists.ban"
                    : "admins.receptionists.unban",
                selectedReceptionist.value.id,
            ),
            {
                preserveScroll: true,
...
<template>
    ...
    <div class="space-y-6">
        <section
            class="overflow-hidden rounded-[30px] border border-[#d9e2ec] bg-white shadow-sm"
        >
            <div
                class="flex flex-col gap-6 bg-gradient-to-r from-[#0c1a2e] via-[#10233c] to-[#163152] px-6 py-8 text-white sm:px-8 lg:flex-row lg:items-end lg:justify-between"
            >
                <div class="max-w-3xl space-y-3">
                    <Badge class="border-sky-400/30 bg-sky-400/10 text-sky-100"
                    ...
                    <div class="space-y-2">
                        ...
                        <p
                            class="max-w-2xl text-sm leading-6 text-slate-200/85"
                        >
                            Create receptionist accounts, review access status,
                            and keep front-desk staff management aligned with
                ...
                <div v-if="permissions.can_create" class="shrink-0">
                    <Link :href="route('admins.receptionists.create')">
                        <Button
                            class="h-11 rounded-xl bg-white px-5 text-sm font-semibold text-[#0c1a2e] hover:bg-slate-100"
                        >
                            Add Receptionist
                        </Button>
                    </Link>
            ...
            <div
                class="grid gap-4 border-t border-white/5 bg-[#f7f9fc] px-6 py-6 sm:grid-cols-3 sm:px-8"
            >
                <Card class="rounded-2xl border-[#d9e2ec] shadow-none">
                    <CardContent class="flex items-center justify-between p-5">
                        <div>
                            <p
                                class="text-xs font-semibold uppercase tracking-[0.18em] text-[#5b708b]"
                            >
                                Total
                            </p>
                            <p
                                class="mt-2 text-3xl font-semibold text-[#0c1a2e]"
                            >
                                {{ stats.total ?? 0 }}
                            </p>
                        </div>
        ...
        <section
            class="rounded-[28px] border border-[#d9e2ec] bg-white p-6 shadow-sm"
        >
            <div
            ...
            >
                <div>
                    <h2 class="text-xl font-semibold text-[#0c1a2e]">
                        Receptionists Directory
                    </h2>
                    <p class="mt-1 text-sm text-[#5b708b]">
                        Search by name or email and use the actions column to
                        edit, ban, unban, or delete.
                    </p>
                </div>
            ...
            <div
                class="mt-6 flex flex-col gap-4 border-t border-[#e7edf5] pt-5 sm:flex-row sm:items-center sm:justify-between"
            >
                <div class="flex items-center gap-2 justify-between w-full">
                    ...
                    <span class="px-2 text-sm font-medium text-[#5b708b]">
                        Page {{ receptionists.current_page }} of
                        {{ receptionists.last_page }}
                    </span>
        ...
        <AlertDialogContent>
            <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                    ...
                    <span
                        v-if="selectedReceptionist"
                        class="mt-2 block font-medium text-slate-700"
                    >
